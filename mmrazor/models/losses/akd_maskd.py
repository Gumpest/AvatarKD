...
class AKD_MasKDLoss(nn.Module):

    def __init__(self, channels, num_tokens=6, weight_mask=True, custom_mask=True, custom_mask_warmup=1000, pretrained=None, loss_weight=1., drop_rate=0.1):
        super().__init__()
        ...
        self.Uncertain = nn.ModuleList([
            nn.BatchNorm2d(c, affine=False) for c in channels]
        )
    ...
    def forward(self, y_s_list, y_t_list):
        ...
        losses = []
        for y_s, y_t, Uncertain, mask_module in zip(y_s_list, y_t_list, self.Uncertain, self.mask_modules):
            # 1. multi-tea
            y_t = F.dropout(y_t, p=self.drop_rate, training=True)
